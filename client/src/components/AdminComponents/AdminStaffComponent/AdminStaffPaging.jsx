...
function AdminStaffPaging(props) {
    const { dataValue } = props;
    // State phân trang
    const [previous, setPrevious] = useState(null);
    const [elmsPhanTrang, setElmsPhanTrang] = useState(null);
    const [next, setNext] = useState(null);
    const [notFound, setNotFound] = useState('Không tìm tài khoản nhân viên nào!');
    ...
    useEffect(() => {
        ...
        if (dataValue.listNV && dataValue.listNV.length > 0) {
            ...
            setNext(null);
            setPrevious(null);
            setNotFound('Không tìm thấy tài khoản nhân viên nào!');
        }
    }, [dataValue])
    return (
...
export default AdminStaffPaging
